...
/ .state.CurrentOrder,:0^.util.testutils.makeDefaultsRecords[
/     `.state.CurrentOrders;
/     `time`accountId`side`otype`price`leaves`reduce;
/     flip(5#z;5?0 1;5?-1 1;5#1;{1000+x}'[til[5]];5#100;`boolean$(5?0 1))]

// Inventory
// ----------------------------------------------------------------------------------------------------------------->

.qt.Unit[
    ".state.allOpenInventory";
    {[c]
        p:c[`params];
