...
.quanto.account.ExecCost         :{[price;qty]
...
.quanto.account.AvgPrice         :{[isignum;execCost;totalEntry]
     
    };

// Given the current Inventory state, this function will derive the
// unrealized pnl that the inventory has incurred.
.quanto.account.UnrealizedPnl    :{[amt;isignum;avgPrice;markPrice;faceValue] // todo return multiplier val
...
// back to the balance.
.quanto.account.RealizedPnl      :{[fillQty;fillPrice;isignum;avgPrice;faceValue]
     
    };

// Derive the maintenence margin i.e. the amount of margin required to
// keep the specified inventory open. 
.quanto.account.MaintMarginReq      :{[]
...
.quanto.account.InitMarginReq       :{[]

    };

// Derive the maintenence margin i.e. the amount of margin required to
// keep the specified inventory open. 
.quanto.account.MaintMargin      :{[]
...
.quanto.account.LiquidationPrice :{[a;iB;iL;iS;ins]
...
.quanto.account.BankruptcyPrice  :{[account;iB;iL;iS;ins] // TODO check
...
.quanto.account.AdjustOrderMargin       :{[price;delta;markPrice;isign]
...
.quanto.account.incFill                 :{[price;qty;account;inventory]
...
/  @param inventory (Inventory) The inventory that is going to be added to.
/  @return          (Inventory) The new updated inventory
.quanto.account.redFill                 :{[price;qty;account;inventory]
...
/  @param inventory (Inventory) The inventory that is going to be added to.
/  @return          (Inventory) The new updated inventory
.quanto.account.crsFill                 :{[price;namt;account;inventory]
...
// 
.quanto.account.ApplyFill               :{[a;iB;iL;iS;fill]
...
.quanto.account.UpdateMarkPrice         :{[markPrice;instrument;a]
...
.quanto.account.ApplyFunding        :{[fundingRate;instrument;account]
