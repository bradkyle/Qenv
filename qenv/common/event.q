...
.event.Order     		:{x:.event.prep[x];([]kind:`order;time:flip x`time;datum:x`oId`time`aId`amt`rpnl`avgPrice`upnl;aId:7h$x`aId)}
.event.Deposit 			:{x:.event.prep[x];([]kind:`deposit;time:flip x`time;datum:x`aId`time`dep;aId:7h$x`aId)}
.event.Withdraw 		:{x:.event.prep[x];([]kind:`withdraw;time:flip x`time;datum:x`aId`iId`time`wit;aId:7h$x`aId)}
