...
TESTSTATE   :`READY`PASS`FAIL`SKIP`ERROR`COMPLETE`SILENT;
...
Unit        :{[name;testFn;formFn;cases;hooks;dscr]
    ...
    test:cols[.qt.Test]!((.qt.testId+:1);name;`UNIT;`READY;1b;dscr;testFn;formFn;0;0;hooks[0];hooks[1];hooks[2];hooks[3];.z.z;.z.z;.z.f);
    `.qt.Test upsert test;
...
Integration    :{[namespaces;testFn;formFn;cases;hooks;dscr]
    if[count[namespaces]<=1;'INVALID_INTEGRATION_NAMESPACES];
    $[not null[`$dscr];dscr:`$dscr;dscr:`$""];
    name: `$(sv[":";namespaces]);
...
runTest         :{[test]
    .qt.beforeHooks[""];
    cases:select from 0!.qt.Case where state in `READY`SILENT, testId=test[`testId];
    test[`beforeAll][];
    test[`start]:.z.z;
...
.qt._RunTests :{[vbs]
    ...
    .qt.pntTest each 0!$[vbs;
        .qt.Test;
        select from ej[`testId;.qt.Test;select xc:count i by testId from .qt.Case where testId.visible and not[state in `SILENT`SKIP]] where xc>0];

    show 400#" ";
    show 99#"#";show (38#" "),"TESTS COMPLETED";show 99#"#";
    show select count i by state from .qt.Case;
    show 400#" ";
    /* show select count i by state, string[testId.name] from .qt.Case where state <> `PASS; */
    };
...
SkpBn       :{[testname]
    c:`$testname;
    .qt.beforeHooks:{update state:`.qt.TESTSTATE$`SKIP from `.qt.Case where not[testId.name in x];y}[c];
    };

ShowOnly       :{[testname]
    c:`$testname;
    .qt.afterHooks:{update visible:0b from `.qt.Test where not[name in x];y}[c];
    };
// Mock
// ======================================================================>
...
MM :{[target;replacement;ecalled;enumCalls;eCalledWith;case]
    ...
    .qt.mockId+:1;
    mck:(.qt.mockId;case[`testId];case[`caseId];`MOCK;`.extern;target;get target;replacement;0b;0;0;0b;0;ecalled;enumCalls;eCalledWith);
    .test.mck:mck;
    `.qt.Mock insert mck; 
    // Replace target with mock replacement
    target set wrapperFn[replacement;.qt.mockId];
    :?[`.qt.Mock;enlist(=;`mockId;.qt.mockId);0b;()]
    };
...
MS :{[fn;case]
    .qt.mockId+:1;
    mck:(.qt.mockId;case[`testId];case[`caseId];`MOCK;`.extern;`lambda;fn;fn;0b;0;0;0b;0;0b;0;());
    `.qt.Mock insert mck;
    (.qt.mockId;wrapperFn[fn;.qt.mockId])
    };
...
AT  :{[func; arg; exception; msg; case]
    ...
    ass:cols[.qt.Assertion]!((assertId+:1);case[`testId];case[`caseId];`THAT;state;`$(raze string'[raze msg]);enlist[actual];`throws;enlist[exception]);
    `.qt.Assertion upsert ass;
    :{.qt.Assertion@x}.qt.assertId;
...
// simpel wrapper function that checks if an error should be thorwn
CheckErr :{[fn;args;err;case]
        $[count[err]>0;[
            AT[fn; args; err; ""; case];
        ];[
            :fn[args];
...
.qt.generalParams :{`mocks`args`err`eRes!x}; 
...
Ma      :{[case;mId]
        m:.qt.Mock@mId;
        t:string[m[`targetPath]];
        .qt.A[m[`called];=;m`ecalled;t," called";case];
        .qt.A[m[`numCalls];=;m`enumCalls;t, " numCalls";case];
        .test.m:m;
        .test.mId:mId;
        if[count[m`eCalledWith]>0;[
            .qt.AAll[exec invokedWith from .qt.Invocation where mockId=mId;m`eCalledWith;t," invokedWith";case];
        ]];
        
    };

// checks that a mock conforms to the parameters provided
// used for checking a single mock
// TODO add additional logic
// Checks that the .engine.model.order.Order table matches the orders
// that are provided.
/  @param x (Mock/List) The mocks that are to be checked
...
mM :{[case;target;replacement]
    .qt.M[target;replacement;case] 
    };

// Creates a set of mocks according to a lsit proviced 
// the mocks assume a generalized format of 
// was_called;num_calls; called with; resfn
mmM :{[case;mocks]
    $[count[mocks]>0;[
        /* show mocks; */
        x:{.qt.MM[y[0];y[1];y[2];y[3];y[4];x]}[case]'[mocks];
        :0!raze[x];
        ];:()];
    };
...
RunUnit : {[c;fn] 
    p:c`params;
    mocks : .qt.mmM[c;p`mocks];
    res: .qt.CheckErr[fn;p`args;p`err;c];
    .test.mocks:mocks;
    if[count[mocks]>0;.qt.Ma[c]'[mocks`mockId]];
    .qt.A[res;~;p[`eRes];"res";c];
    };


/ // Utilities and datums 
/ // ======================================================================>
...
Unit[
    ...
    ({};{};{};{});
    "unit test for testing quantest functionality"]; 
